...
mod tests {
    ...
    // The kernel's own numbers, under a name that says whose they are. Every `uapi::`
    // reference below is a bindgen constant generated from this host's UAPI headers, and
    // the point of the alias is that a reader can see at a glance which side of an
    // assertion is the authority.
    ...
    /// The same argument as the widths above, for the *numbers* rather than the layout.
    ///
    /// `schema::control::KnownFlag` transcribes thirteen kernel ABI bits by hand, and it
    /// has to: `webcam-handler-schema` is the vocabulary crate and T6's purity wall keeps
    /// it clear of the V4L2 backend, so it cannot name a bindgen constant. This crate can,
    /// and until 2026-08-17 nobody did — the constants were linked in and never compared
    /// (the G6 review's L11, note **N228**). A transposed bit here is not a compile error
    /// and not a decode error: it is `Inactive` read as `Slider`, which is an automation
    /// partner's ownership of a control going unnoticed \[PF:3\].
    ///
    /// Exhaustive by construction. `KnownFlag::ALL` is generated by `bit_vocabulary!` from
    /// the same list the bits are declared in, so a fourteenth flag arrives in the loop
    /// below and does not compile until somebody names the kernel constant it is.
    #[test]
    fn every_control_flag_bit_the_schema_names_is_the_bit_the_kernel_headers_define() {
        for flag in KnownFlag::ALL.iter().copied() {
            let (name, kernel) = match flag {
                KnownFlag::Disabled => ("V4L2_CTRL_FLAG_DISABLED", uapi::V4L2_CTRL_FLAG_DISABLED),
                KnownFlag::Grabbed => ("V4L2_CTRL_FLAG_GRABBED", uapi::V4L2_CTRL_FLAG_GRABBED),
                KnownFlag::ReadOnly => ("V4L2_CTRL_FLAG_READ_ONLY", uapi::V4L2_CTRL_FLAG_READ_ONLY),
                KnownFlag::Update => ("V4L2_CTRL_FLAG_UPDATE", uapi::V4L2_CTRL_FLAG_UPDATE),
                KnownFlag::Inactive => ("V4L2_CTRL_FLAG_INACTIVE", uapi::V4L2_CTRL_FLAG_INACTIVE),
                KnownFlag::Slider => ("V4L2_CTRL_FLAG_SLIDER", uapi::V4L2_CTRL_FLAG_SLIDER),
                KnownFlag::WriteOnly => {
                    ("V4L2_CTRL_FLAG_WRITE_ONLY", uapi::V4L2_CTRL_FLAG_WRITE_ONLY)
                }
                KnownFlag::Volatile => ("V4L2_CTRL_FLAG_VOLATILE", uapi::V4L2_CTRL_FLAG_VOLATILE),
                KnownFlag::HasPayload => (
                    "V4L2_CTRL_FLAG_HAS_PAYLOAD",
                    uapi::V4L2_CTRL_FLAG_HAS_PAYLOAD,
                ),
                KnownFlag::ExecuteOnWrite => (
                    "V4L2_CTRL_FLAG_EXECUTE_ON_WRITE",
                    uapi::V4L2_CTRL_FLAG_EXECUTE_ON_WRITE,
                ),
                KnownFlag::ModifyLayout => (
                    "V4L2_CTRL_FLAG_MODIFY_LAYOUT",
                    uapi::V4L2_CTRL_FLAG_MODIFY_LAYOUT,
                ),
                KnownFlag::DynamicArray => (
                    "V4L2_CTRL_FLAG_DYNAMIC_ARRAY",
                    uapi::V4L2_CTRL_FLAG_DYNAMIC_ARRAY,
                ),
                KnownFlag::HasWhichMinMax => (
                    "V4L2_CTRL_FLAG_HAS_WHICH_MIN_MAX",
                    uapi::V4L2_CTRL_FLAG_HAS_WHICH_MIN_MAX,
                ),
            };
            assert_eq!(
                flag.bit(),
                kernel,
                "{flag:?} claims {name}, which this host's headers put at {kernel:#06x}"
            );
        }
    }

    /// The other half of the same class, found while landing the one above: every kernel
    /// number this *module* spells as a literal, against the constant its doc comment
    /// cites (note **N228**).
    ///
    /// Driven from the kernel side, which is AGENTS rule 4 applied to a header instead of
    /// to a device: the loop asks what `V4L2_CTRL_TYPE_RECT` is and then what this build
    /// does with it, rather than asking this build what it believes and looking for
    /// agreement.
    ///
    /// **The residual, stated rather than implied:** a fifteenth `ControlType` variant
    /// added to the schema without a row here is not caught, because the enum has no
    /// generated `ALL` to iterate — `Unknown { raw }` is a payload-carrying arm and does
    /// not fit one. What *is* caught is the case that matters more, which is a named
    /// variant sitting on the wrong discriminant.
    #[test]
    fn every_kernel_number_this_module_transcribes_equals_the_constant_it_cites() {
        let words: [(&str, u32, u32); 10] = [
            (
                "V4L2_CTRL_TYPE_INTEGER_MENU",
                CTRL_TYPE_INTEGER_MENU,
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_INTEGER_MENU,
            ),
            (
                "V4L2_CTRL_TYPE_INTEGER64",
                CTRL_TYPE_INTEGER64,
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_INTEGER64,
            ),
            (
                "V4L2_FRMSIZE_TYPE_DISCRETE",
                FRMSIZE_TYPE_DISCRETE,
                uapi::v4l2_frmsizetypes_V4L2_FRMSIZE_TYPE_DISCRETE,
            ),
            (
                "V4L2_FRMSIZE_TYPE_CONTINUOUS",
                FRMSIZE_TYPE_CONTINUOUS,
                uapi::v4l2_frmsizetypes_V4L2_FRMSIZE_TYPE_CONTINUOUS,
            ),
            (
                "V4L2_FRMSIZE_TYPE_STEPWISE",
                FRMSIZE_TYPE_STEPWISE,
                uapi::v4l2_frmsizetypes_V4L2_FRMSIZE_TYPE_STEPWISE,
            ),
            (
                "V4L2_FRMIVAL_TYPE_DISCRETE",
                FRMIVAL_TYPE_DISCRETE,
                uapi::v4l2_frmivaltypes_V4L2_FRMIVAL_TYPE_DISCRETE,
            ),
            (
                "V4L2_FRMIVAL_TYPE_CONTINUOUS",
                FRMIVAL_TYPE_CONTINUOUS,
                uapi::v4l2_frmivaltypes_V4L2_FRMIVAL_TYPE_CONTINUOUS,
            ),
            (
                "V4L2_FRMIVAL_TYPE_STEPWISE",
                FRMIVAL_TYPE_STEPWISE,
                uapi::v4l2_frmivaltypes_V4L2_FRMIVAL_TYPE_STEPWISE,
            ),
            (
                "V4L2_CAP_TIMEPERFRAME",
                CAP_TIMEPERFRAME,
                uapi::V4L2_CAP_TIMEPERFRAME,
            ),
            (
                "V4L2_BUF_FLAG_ERROR",
                BUF_FLAG_ERROR,
                uapi::V4L2_BUF_FLAG_ERROR,
            ),
        ];
        for (name, ours, kernel) in words {
            assert_eq!(ours, kernel, "{name} is {kernel} in this host's headers");
        }

        // `ControlType`'s fourteen named discriminants, which live in the schema for the
        // same reason the flags do and were transcribed the same way.
        let types: [(&str, u32, ControlType); 14] = [
            (
                "INTEGER",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_INTEGER,
                ControlType::Integer,
            ),
            (
                "BOOLEAN",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_BOOLEAN,
                ControlType::Boolean,
            ),
            (
                "MENU",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_MENU,
                ControlType::Menu,
            ),
            (
                "BUTTON",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_BUTTON,
                ControlType::Button,
            ),
            (
                "INTEGER64",
            ...
            (
                "CTRL_CLASS",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_CTRL_CLASS,
                ControlType::ControlClass,
            ),
            (
                "STRING",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_STRING,
                ControlType::String,
            ),
            (
                "BITMASK",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_BITMASK,
                ControlType::Bitmask,
            ),
            (
                "INTEGER_MENU",
            ...
            (
                "U8",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_U8,
                ControlType::U8,
            ),
            (
                "U16",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_U16,
                ControlType::U16,
            ),
            (
                "U32",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_U32,
                ControlType::U32,
            ),
            (
                "AREA",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_AREA,
                ControlType::Area,
            ),
            (
                "RECT",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_RECT,
                ControlType::Rect,
            ),
        ];
        for (name, raw, want) in types {
            assert_eq!(
                ControlType::from_raw(raw),
                want,
                "V4L2_CTRL_TYPE_{name} is {raw} and must decode to {want:?}"
            );
            assert_eq!(want.to_raw(), raw, "V4L2_CTRL_TYPE_{name} round-trips");
        }

        // And AGENTS rule 6's half, from the same authority: a type this build has no name
        // for is carried with its discriminant rather than corrected into a neighbour. The
        // three are stateless-codec control types on this host's headers, which is the
        // shape a kernel upgrade delivers more of.
        for (name, raw) in [
            (
                "HDR10_MASTERING_DISPLAY",
                uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_HDR10_MASTERING_DISPLAY,
            ),
            ("H264_SPS", uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_H264_SPS),
            ("AV1_FRAME", uapi::v4l2_ctrl_type_V4L2_CTRL_TYPE_AV1_FRAME),
        ] {
            assert_eq!(
                ControlType::from_raw(raw),
                ControlType::Unknown { raw },
                "V4L2_CTRL_TYPE_{name} has no name here and must arrive as its number"
            );
        }
    }
