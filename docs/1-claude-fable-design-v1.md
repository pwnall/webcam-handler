# Design Document: webcam-handler — Architecture (v1)

Doc 1 in the webcam-handler series, **v1 — initial revision**. Status: current. Companion
documents: the phased implementation plan (docs/2), the code review rubric (docs/3), the
automated quality gates (docs/4), and AGENTS.md (docs/5, deployed at the repository root).

**What this document was built from.** Three inputs, in decreasing order of authority:

1. **Hardware probes run during design** (2026-08-07, kernel 7.0.0-29-generic, rustc
   1.97.1) against two attached cameras — a Chicony integrated camera (04f2:b83c, which is
   *two* logical cameras: RGB + IR) and an OBSBOT Tiny 3 (a PTZ camera). The probe record is
   §1.2; findings are cited as **[PF:n]** throughout. Where a probe finding contradicts a
   crate's documentation or the V4L2 specification, the probe wins (E1, §2.12).
2. **The v4l2-webcam agent skill** (`vendor/v4l2-webcam-skill/`), which records the manual
...
3. **Crate research with adversarial license audits** (2026-08-07, crates.io + repository
   verification). Versions and licenses in §2.8 are as verified on that date.
...
Measured 2026-08-07 on the hardware named above. These are load-bearing citations, not
anecdotes; rubric rule 5 (docs/3) and AGENTS.md rule 4 require new hardware behavior to
land here (or in the implementation notes once the repo exists) before code
special-cases it.
...
| V4L2 backend | `webcam-handler-v4l2` | T1/T2 impl: sysfs+devnode enumeration, raw QUERY_EXT_CTRL control layer, mmap streaming via `v4l`, uevent hotplug [Linux I/O] |
| Fake backend | `webcam-handler-fake` | T1/T2 impl: scriptable in-memory cameras + device-profile replay (T3) [pure] |
| API | `webcam-handler-api` | the jsonrpsee `#[rpc(server, client)]` trait over schema types + the error-code registry (T5) [pure] |
...
interface (or, for non-USB V4L2 devices, a media controller device / driver-reported bus
info) [PF:7]. The capture node is the group member whose `device_caps` contain
...
- `CameraId` — the name RPC calls and CLI arguments use. Grammar: `cam:<card-slug>[-<n>]`,
  ...
  cost agents nothing.
- `CameraFingerprint` — best-effort stable across replug/reboot: bus path, USB VID:PID,
...
interfaces with no video4linux binding and reports "USB camera present without a V4L2
driver" when it finds one — the skill's `lsusb` triage, built in (§1.1).
...
2. **Empirical discovery** (`controls --discover-pairs`, and automatically at calibration
   start): toggle each automation-shaped control and diff INACTIVE flags across the set
   [PF:3]; discovered pairs are recorded in the device profile with provenance
   `measured`, and trump the table on conflict (E1).
...
reports per-control `{requested, applied}` like any write, and INACTIVE controls are
restored on a second pass after their automation partner is disabled, or reported as
unrestorable with the reason. (In §5's terms: every sweep/guarded operation restores by
default, and the R3 hardware suites assert the restoration.)
...
[PF:9, PF:11]) or `settle_for(duration)`, both bounded by a deadline. Buffers are mmap'd;
...
sample photo is self-describing without its session file. Every photo path — verbatim
...
full bind × token matrix, stated once (docs/2 G5 and the docs/4 token gate cite this
...
**D13 — The error vocabulary.** `webcam-handler-schema` defines the closed typed registry; every
variant carries what the caller needs to act: `DeviceGone`, `Busy { holders:
Vec<{pid, comm}> }` (diagnosed from `/proc/*/fd` — the `fuser` replacement; killing is a
...
named pid no longer holds the device). The registry is **errors only**: a clamp
...
from failure. JSON-RPC error codes map 1:1 from this registry in `webcam-handler-api`;
...
pub trait CameraBackend: Send + Sync {
    fn name(&self) -> &'static str;                       // "v4l2", "fake"
...
- **Controls**: raw `VIDIOC_QUERY_EXT_CTRL`/`QUERYMENU`/`G_EXT_CTRLS`/`S_EXT_CTRLS`
  ...
  bypassed entirely (PF:1).
- **Streaming**: the `v4l` crate's mmap `Stream` (validated, PF:9), format negotiation via
  `set_format`/`set_params` with the negotiated result surfaced (D5).
...
- **One module holds every `unsafe` block**: `crates/backends/v4l2/src/sys/`. Every
  *other crate* is `#![forbid(unsafe_code)]` at its root; within `webcam-handler-v4l2` —
  ...
  (docs/4). The uevent socket code lives inside the same module for the same reason.
...
- **Miri runs the unsafe-adjacent pure units** — the raw-struct→`ControlDesc` decoding
  ...
  without a device (docs/4 commissions the job).
...
Purity walls (T6): `schema`, `imaging`, `fake`, `api`, `cli-core` link no
tokio/axum/hyper; only `daemon` links the web stack; `client` links no backend and no
engine (the thin-client property). Those are **linkage** walls, gate-asserted from
`cargo metadata` (docs/4). The *behavioral* halves — the pure crates touch no filesystem
outside explicit inputs, only `v4l2` touches `/dev` and `/sys`, only `engine` and the
composition roots touch the state dir — are review-held, stated as such so the gate's
green is not read as covering them (docs/4 records the limit). The backend trait means
`engine` never names V4L2; a grep gate enforces that one (docs/4).
...
feature that pulls it is the feature-posture gate's subject — docs/4 owns that half),
...
(links LGPL/GPL FFmpeg), `x264`/`x265` bindings (GPL), `dssim` (AGPL), and `jpeg-encoder`/
`turbojpeg`/`mozjpeg` (IJG term — §8 item 1; the default stack does not need them). TLS
...
imageproc 0.27 (MIT), little_exif 0.6 (MIT/Apache), y4m 0.8 (MIT), clap 4 + complete +
...
libsystemd), serde/serde_json, toml 1 (config), schemars 1 (MIT), directories 6, tempfile
...
commissioning time, docs/4). Test-time external tooling, outside the shipped license
...
A second copy of any of these is a review finding (docs/3 B-checklist); the gates enforce
the mechanically checkable ones (docs/4).
...
- **R2 — kernel-virtual (opportunistic gate):** the `vivid` virtual capture driver
  exercises the real ioctl layer without hardware; runs where the module is loadable
  (dev boxes, self-hosted runner), auto-skips elsewhere **with the skip counted and
  named** — a rung that silently becomes a no-op on every runner is the predecessor's
  "check that cannot fail" defect class.
...
- `corpus/profiles/` holds device profiles (the T3 format) captured by the tool itself,
  ...
  (PF:1–PF:9, PF:12) is representable in — and asserted from — at least one committed
  profile (the sparse menu [PF:2] and the out-of-range default [PF:5] are regression
  fixtures, not prose). PF:10 lands in the build docs and PF:11 in the fake's settle
  model — they are not profile-shaped, and the corpus-floor gate counts only what is.
...
- `corpus/images/` holds **synthetic fixtures only** — generated patterns with known
  ...
  this is a content check, not a convention (docs/4).
- Fixtures enter tests as bytes; a fixture nobody loads is dead corpus (gate: corpus
  floor counts, docs/4).

### 3.3 The structural-gap register (v1, honest)

What this suite cannot reach, named up front:
...
Lives in **docs/2 (implementation plan v1)** — phases P0–P6 with gates G0–G6, each gate a
named, counted, re-runnable criterion set consuming this document's decision registry.
...
- **The `v4l` crate is semi-dormant (bus factor 1, last release 2023).** Mitigated: pinned
  version; our exposure is narrow (mmap streaming + format ioctls — the control layer is
  already ours, PF:1); `v4l2r` is the named migration target behind T2; worst case the
...
- **Kernel/driver variance dwarfs our test matrix.** The design leans on D2/D3 doctrine
  ...
  norm — four of twelve PF findings are devices contradicting their own declarations.
...
9. **An MCP server surface.** Agents reach the tool today through the CLIs and the
   JSON-RPC daemon, plus the generated agent usage guide (docs/2 P6); a Model Context
