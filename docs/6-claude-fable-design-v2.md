...
| Change | Where | Source |
|---|---|---|
| The error registry grows from fourteen variants to eighteen: `CameraUnknown`, `CameraAmbiguous`, `DeviceIo`, `StorageIo` complete it. A nineteenth, `Unimplemented`, was transitional and died at P4d as scheduled, taking D13's lowest code with it and moving no other | D13 | N4, N6 (retired) |
...
A nineteenth, `Unimplemented { operation, arrives_in }`, existed and was **transitional**
(N6, retired at P4d): the T1/T2 traits are total from P0 while the plan lands their methods
across phases, so a method whose phase had not arrived needed an answer that was neither a
panic [PF:1], nor the kernel's fault (`DeviceIo`), nor a capability lie (E3's whole
subject). It was pinned to the one `unimplemented_surface()` list, whose test each landing
phase had to edit; when `watch` landed at P4d the list was empty and the variant was
deleted, driven out by the exhaustive `Error::kind()` match exactly as scheduled. It held
D13's **lowest** code deliberately, so the deletion raised one endpoint of the block and
left the other eighteen wire codes byte-identical.
...
and `Error::kind()` is an exhaustive match, so a nineteenth variant does not compile
...
- **The traits are total from P0** even though the plan lands their methods across
  phases. While a phase was outstanding its methods answered `Error::Unimplemented` (D13,
...
webcam-handler/
  ...
  crates/
    ...
    backends/v4l2/      # webcam-handler-v4l2    [v4l (default features ONLY), kobject-uevent, libc,
                        ...
                        #              had a logging edge, and hotplug::Counts is why it needs none]
...
10. ~~**Does the uevent socket actually need `CAP_NET_ADMIN`?**~~ **Answered at P4d: it
    does not** (PF:21). N8 granted it on an unverified prediction — the probe that would
    have answered it was blocked. Taken on this kernel, `7.0.0-29-generic`, by a process
    with an empty effective capability set: the `NETLINK_KOBJECT_UEVENT` group-1 bind
    succeeds, and three separate `uvcvideo` cycles delivered every one of their 56 packets
    to it with `ENOBUFS` never raised — binding and receiving being two claims, both
