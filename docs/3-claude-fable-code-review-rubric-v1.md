# webcam-handler — Code Review Rubric (v1)

*Doc 3 in the webcam-handler series, **v1 — initial revision**. Grounded on the design
(docs/1) and enforced mechanically where possible by the gate suite (docs/4). The standing
...
gate.** v1 is written before the repository exists; its principles are transfers and
probe findings, not yet local case law. Each reconciliation replaces speculation with
citations or deletes rules that never fire.*
...
surfaced defect yet; **[PF:n]** surfaced by the design-phase hardware probes (docs/1
§1.2) — measured on real cameras, the closest thing v1 has to local case law. Once the
repo exists, repo-surfaced findings are tagged **[S:Nx]** citing implementation-note
entries.
...
6. **Every gate predicate carries a self-test that proves both directions.** A gate with
   ...
   and a fixed-size array does not substitute. [SB]
...
9. **The fake is faithful, driven, and failure-injectable — and resemblance is owned by
   the real side.** [SB A9/A20][E5] The fake replays captured profiles; its behavioral
   ...
   real device exhibits is a finding against the fake.
...
12. **Privacy is a correctness property.** [design §5] A camera frame may contain a
    ...
    content gate (docs/4) covers the repository half.

13. **A probe carries one variable.** [SB] Empirical pair discovery toggles one
...
- [ ] The AVI muxer's committed byte fixtures cover header/movi/idx1; its output re-parses
      ...
      complete frame. `test`
- [ ] EXIF stamps are read back with the independent reader crate in tests, not assumed
      from the writer's return. `test`
...
- [ ] The T5 trait is the only wire surface; every method has an integration test,
      ...
      methods, and docs/4's method-count-walk row is the authoritative mechanism (a new
...
- [ ] R3 suites are `#[ignore]`d, recipe-named, restore what they touch, and skip
      motor-moving sweeps without `--allow-motion`. `CI` `test`
...
- [ ] **Miri runs the unsafe-adjacent pure units** (raw-struct decoding over captured
      bytes); code in the sys module is shaped to keep that population growing rather
      than shrinking. [V] `CI`
...
Deployable contents live in docs/4. The doctrine: every defect family fails a lint, a CI
...
- **Phase preflight**: name the gate in force (G0–G6, docs/2) and what green does not
...
- **Reconciliation at each phase gate** — the meta-rule. v2 of this rubric is due at the
  first gate whose defects it failed to predict, and no later than G4.
