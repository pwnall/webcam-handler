...
Doc 15 in the webcam-handler series, **v3 — second revision**. Status: **adopted** at docs/13
P7a (commit `796babb`, 2026-08-18); supersedes docs/9 (v2), which now lives under
`docs/historical/`. Gates for docs/14 (rubric v3, Part D), consumed by docs/13's phase gates.
The convention in force from the first commit is unchanged: **the repository's files are
authoritative; this document records the commissioned set, deltas, and rationale.** A drift
between this document and the repo's files is a defect in whichever one the evidence says is
wrong.
...
| `avi-reparse-is-independent.sh` | the muxer and its re-parser share no code, constant or helper — the sibling stem looked for in a path position, over statements joined and flattened through the shared `rust-imports.awk` rather than over raw lines |
...
| `counted-selections.sh` | every criteria row still selects more than zero tests, every named branch of an alternation with it, every `command` row's script present and executable, and every phase the table declares carrying both a `run-all.sh` row and a `selftest.sh` row |
...
| `facade-is-the-composition.sh` | the direct CLI's executor crate reaches the engine only through `engine::facade`, over a population derived from the facade's own methods — its imports read first through the shared `rust-imports.awk` and its calls resolved through the local names they bind, so a composition move is the same fact however it is spelled — and a walk over every file under `crates/cli/src`; the ban is on the class of reach and not one spelling of it — every visibility and both import keywords are read through the shared `rust-imports.awk`, joined and flattened before the walk sees them, and the two shapes that cannot reduce to a path (a binding of the crate, a glob of it) are refused with counted populations of their own; the two policy lists — the lifecycles D18 excludes and the two root-only reaches — are checked in every direction there is, the lifecycles against the executor's own doc sentence among them |
| `facade-stability-table-sync.sh` | D18's stability table is reconciled against the crates it names, both ways: every module in exactly one column, every named module still declared, every named crate a crate — every engine module the facade's imports, the signatures of every associated item its `impl` blocks make reachable, or its module-scope public items name sits in the **Yes** column, and so does every module the public surface of a **Yes** module itself forces on a holder, with an import that yields no module refused rather than shrinking either population — and D18's own bullet in docs/12 names the table rather than restating it |
...
| `profile-partition-is-closed.sh` | every partition D15 closes **by destructuring** is still destructured — the mechanism the compiler enforces, which a pattern "simplified" into field access silently reopens while compiling perfectly; field names derived from each struct's own declaration |
...
| ~~**Projection closure**~~ (landed, as a predicate rather than a fixture) | P7c | **delta**: this row asked for a compile-fail fixture, and what landed is `profile-partition-is-closed.sh` — because the compiler *already* refuses a field nobody sided, and a `trybuild` harness would therefore be a test that the compiler works. What can go wrong without anyone noticing is the *mechanism*: a destructuring "simplified" into field access compiles perfectly and silently reopens the partition, which is what the predicate reads for, over field names derived from each struct's own declaration. The corpus mutual-negative walk landed as asked. **Delta (2026-08-21, the gate-close review):** the row list stopped one struct short of the partition it is about. `camera.rs` declares two functions called `differing_fields` and the header named that ambiguity while declaring a row for one of them — the `CameraInfo` one, which destructures — and the neighbour it delegates the identity half to read its five fields through `self.`, so it could not be given a row at all: claim 3 fails a function carrying zero patterns, and the gate was green here for the reason it exists to catch. A sixth field seeded onto `CameraFingerprint` compiled, passed all 1688 tests and passed this predicate, and two fingerprints differing only in it answered `matches = true` — which is what `engine::snapshot::restore` decides `Error::FingerprintMismatch` from, so AGENTS rule 8's mechanical half would have waved a snapshot onto a camera the new field says is a different device. The comparator destructures now and carries the fifth row, with one fail arm per branch (note **N323**) | a new invariant field silently joining neither side of the identity/device partition, **and** a pattern quietly stopping being one; and, since the delta, the same field arriving on the identity half itself |
| ~~**`facade-is-the-composition.sh`**~~ (landed) | P7d | the CLI executor's only engine reach is the facade, population derived from the facade's exports; plus the one-time byte-equivalence criterion at introduction. **Delta (2026-08-20):** as commissioned it banned one spelling — the walk matched `engine::[a-z_]…`, so a grouped `use engine::{pairing, write};` yielded zero reaches and the predicate passed with a summary byte-identical to the unseeded tree (note **N269**). **Second delta, same day:** the repair itself banned one spelling of an *import* — `pub(crate) use`, `extern crate … as`, `use engine::*;` and a bypass moved into a second file of the same crate all passed, each with a byte-identical summary (note **N271**). It now reads every import through the shared `rust-imports.awk`, walks every file under `crates/cli/src`, refuses a binding of the crate and a glob of it with counted populations of their own, derives the encapsulated set from every method of `impl Facade` rather than the exported ones alone, holds each excluded lifecycle to still being reached, and reconciles the policy list against the executor's own doc sentence both ways **Third delta (2026-08-21, the gate-close review):** the *facade* half of the derivation still read one spelling of a call. `encapsulates` was the set of modules named in a fully-qualified `crate::<module>::<fn>(`, so adding `use crate::resolve;` and dropping two prefixes took the population from seven modules to six with the run printing the smaller number, naming no sentence and exiting 0 — after which the executor assembled `engine::resolve::list` by hand and passed with a summary item-for-item the unseeded tree's. It reads the facade's own imports through the same `rust-imports.awk` now and resolves calls through the local names they bind, `as` renames included, with a glob of this crate and an import yielding no module refused rather than shrinking the set. In the same commit, the note listing exports the CLI never reaches stopped being wrong in both directions: it matched `facade.<verb>(` alone, so `Facade::new` — the composition root's one construction — was structurally permanent in it, while the header's hand-written list of that population named `open_id`, which the CLI reaches (note **N315**) **Fourth delta (2026-08-21, the review of that repair):** the resolution was gated on the literal `crate::`, so `use super::resolve;` and `use crate::resolve::{self as r};` were neither resolved nor refused — each took the population from seven to six, printed the smaller number, named no sentence, and passed a hand-assembled `engine::resolve::list` on the next run. `rust-imports.awk` rewrites `super::` to `crate::` and fills the rename the flattener erases, `self::` is a counted refusal rather than an invented module, and both predicates' yields-no-module branches ask `wch_names_this_crate` instead of looking for one prefix (note **N328**) | the facade and the CLI drifting into siblings — the FR's own upgrade-risk, inverted onto us; and, since the deltas, that same drift wearing a brace, a visibility, a glob, a second file or an import line |
| ~~**`facade-stability-table-sync.sh`**~~ (landed) | P7d | docs/14's commissioning line — "the stability table matches the exports both ways" — built in `wire-surface-sync.sh`'s shape: the table parsed out of the facade's module doc and reconciled against every crate it names, in both directions, plus the claim the review found missing (an engine module a facade **signature** forces on a caller must be in the **Yes** column). **Delta (2026-08-20, the day it landed):** it shipped with note **N269**'s hole written fresh — a grouped `use crate::{…}` hid a forbidden module *and* took the surface population from three to one with `gate_require_nonzero` satisfied by the survivor; the walk read only `pub fn`s inside `impl Facade`; and the module derivation could not see an inline `pub mod` (note **N271**). It now shares the import reader, walks the module-scope public items too, refuses an import it can take no module out of, and holds D18's own bullet to naming the table rather than restating it **Second delta (2026-08-21, the gate-close review):** claim 4's widening had enumerated spellings rather than named the class. It carried seven item keywords at module scope and entered only the exact line `impl Facade {`, so every other `impl … {` fell through to a walk that matches nothing at an impl header — and the leak that goes red as a free `pub fn` passed as a `pub fn` on an inherent impl of a module-scope `pub` type, with a counted summary byte-identical to the unseeded tree. Every `impl` block is entered now, an associated item is read when it is written `pub`, and every associated item of a **trait** impl is read whether or not it is, because a trait impl makes its items as public as the trait; the population is every associated signature the language makes reachable, and a private helper on an inherent impl is the green arm that keeps it about the surface (note **N316**) **Third delta (2026-08-21, the review of that repair):** the widened walk still matched one of the three prefixes a path into this crate has, so a **No**-column type reached an embedder from `impl Facade` through `use super::preview::Gap as PreviewGap;` and through an inline `-> Option<super::preview::Gap>`, each with a summary item-for-item the unseeded run's. The rewriting is `rust-imports.awk`'s and is applied inside the path matcher rather than at the import hook alone, so a signature that spells the whole path is read the same way. The same commit landed **claim 6** — every module in a **Yes** row answers for its own public surface, over a population derived from `gate_pub_mods` filtered by the table — which went red on the tree it was written into: `photo::take` took a `crate::actor::OpenCamera` (note **N328**) | the N270 class — a contract table with a hole in it, silent about seven of the engine's twenty modules and reading exactly like a complete one; the shape that hole made possible, a headline verb an embedder cannot call without holding something the table forbids; and, since the deltas, the same table read through a brace or from inside an impl block |
| ~~**Stats criteria + `FrameGap` fault**~~ (landed) | P8a | the accumulator's both-direction arms; the fault's exhaustive-menu membership; the one-home reconciliation with `declared_interval`; truncation stated on the answer. **Delta (2026-08-20):** three of D16's own sentences had no criterion at all. Its two *frame* fields were asserted against one backend, in that backend's own fault suite, so nothing above the ioctl decoder would have gone red if the real driver's sequence went constant — docs/11 H1 verbatim, with the battery's `FRAMES_PER_CYCLE` constant documenting the missing assertion (note **N290**); `wall_clock_skew_us` had no assertion anywhere and `test(/skew/)` selected zero, while sitting inside the mutation floor's `examine_globs` (note **N297**); and `RecordReport::stats` carried an argumentless `#[serde(default)]`, so a document with no stats was the same document as a take that delivered nothing (note **N291**). The clause "the committed schemas carry them" also named an artifact that did not exist: `Frame` is not a wire type, and the three semantics now live on `StreamStats`' own doc, which the bundle carries. **Second delta, same day** (note **N298**): the repair over-reached in three places and under-reached in two. It made a *clock reversal under an advancing sequence* a breach of the frame contract while `StreamStats::clock_reversals` counts the same device event as a measurement — one event, two incompatible answers, and a rig whose uvcvideo timestamps go non-monotonic would have turned the R3 stream arm, the R2 vivid arm and the battery red at once; it asked the ledger for a verdict after an availability refusal, so a camera another process grabbed mid-cycle was answered with a claim about `Frame::sequence` (rule 7, the conversion N138 removed from the same file); and the criterion row claiming both-backend enforcement selected no test that links `webcam-handler-v4l2` — `battery::run` has no real-backend caller — so the R2 rung is a counted criterion beside it now. **Correction (2026-08-21, the closes):** this cell said "selected nine tests" until today. The figure was wrong on the one day it was written and nothing reconciled it; note **N318** took the same figure out of the table's own row and left this copy standing, while what that row holds is the negation rather than a number. The reader that goes red on the next one landed with this close, as `counted-selections.sh`'s count reconciliation. Under-reached: `frames_delivered`'s new committed sentence says "both muxers" and only the AVI muxer had an arm, so the identical hoist in `y4m.rs` left the whole workspace green; the claim moved to `imaging::video`'s `VideoFormat::ALL` × `CapReached::ALL` walk. And the R3 rung's first ordering was a `>=` over two readings of one accumulator, which no reachable state can violate | gap accounting with no driven inverse; a second interval home; silent truncation; and, since the delta, a frame field the real backend could stop filling in silence, a skew nobody measured, and an unmeasured take wearing a measured one's document; and, since the second delta, a measurement carried as a contract, a refusal carried as an incapacity, a both-backend claim no hermetic run walks, a both-muxer claim one muxer holds, and an ordering with no false branch |
...
| ~~**The muxer's import ban**~~ (landed as a delta to `avi-reparse-is-independent.sh`) | P7e/P8d/P9d | the sibling stem looked for in a path position over statements joined and flattened through the shared `rust-imports.awk`, with an import whose braces never close a counted refusal rather than a statement joined into the rest of the module. **Why it is a delta rather than a commissioning:** note **N269** named this predicate as the house precedent the facade gates had missed, and note **N271** then built the shared reader and converted those two — leaving the cited model as the last reader carrying the narrow rule. Measured with this repository's own `rustfmt` and ordinary identifier lengths: the sibling stem landing first on a *continuation* line passed with the `ok`/`note`/`PASS` lines diffing empty against the unseeded run. The two refusals the facade predicates carry are deliberately not transferred and the header says why — `use super::*;` leaves the call site writing `read::…`, and `use super::read::*;` is a violation on the spot (note **N317**) | the two derivations agreeing with each other through an import `grep` could not see; and, in the other direction, the live false positive the punctuation matcher had — an argument named after a module, `fn seam(a: usize, read: usize)`, reported as a grouped import |
| ~~**The phase blocks' own row set**~~ (landed as a delta to `counted-selections.sh`, with nine criteria rows) | P7e/P8d/P9d | every phase the table declares carries both a `run-all.sh` row and a `selftest.sh` row, one sentence per half, over the phases the table declares rather than a list of phase names — so the eleventh phase is inside the rule the day it opens. g8 and g9 were the only blocks without the pair, which is the *first* thing that lands in a block; `just gate-g8` ran four predicates and `just gate-g9` three, none self-tested by the gate adopting them. Said precisely: `just ci` runs both scripts at every green boundary, so what those phases lacked was a gate able to say so rather than the proof itself. The same walk bought g8 the `smoke-hw.sh` row P8c's own **Proves** bullet was owed, g8 and g9 the artifact reconcilers whose committed files P8 and P9 moved, and g7 the `cli-parity.sh` its P7d bullet delegates a claim to in as many words (note **N318**) | a phase closing on a row set nobody walked — and the convention that held for eight blocks and broke on the ninth and tenth |
| ~~**The Yes column's own modules**~~ (landed as claim 6 of `facade-stability-table-sync.sh`) | P7e/P8d/P9d | for every module in a **Yes** row of a crate the table classifies module by module, the `crate::<module>` paths its public item signatures spell — and the ones a private import binds and a signature then writes bare — must also be **Yes**. The same `rust-imports.awk` and the same signature walk claim 4 already runs, pointed at a population derived from `gate_pub_mods` filtered by the table rather than at `facade.rs` alone; a bare `pub use` of the forbidden module's item binds nothing, because that re-export is what makes the type nameable from inside the Yes column. **It went red on the tree it was written into**, which is what commissioning it bought: `photo::take`'s first parameter was `crate::actor::OpenCamera`, a third leak on the same module beside the two the review had found by hand and repaired, and `OpenCamera` is re-exported as `crate::photo::OpenCamera` now (notes **N324**, **N328**) | the finding itself: `engine::photo` sat in the **Yes** column while `take` handed back a `crate::preview` type, took a `crate::actor` one, and `from_capture` demanded a `crate::capture` one — and neither facade predicate has a population that could reach it, because claim 4 reads one file |
| ~~**A criterion row's own count of tests**~~ (landed as a second delta to `counted-selections.sh`) | P7e/P8d/P9d | in a `tests` row, a cardinal that immediately qualifies `test`/`tests` is a claim about how many tests that row selects, and it is compared against the number this predicate already measured and printed three lines above. In digits or in words, hyphenated compounds included, wherever in the cell it stands, through this table's own emphasis markers and through the punctuation the sentence wraps the phrase in, because a ban that names one spelling bans one spelling (note **N249**, rubric A17). A **rate** is not a count and is exempt — `one test per matrix cell` and `one test apiece` say how a row's tests spread over some other population and nothing about how many there are — and the exemption is driven by a passing arm, since an exemption nobody exercises is an exemption nobody checked. The limits are stated in the header rather than left silent, and stated without a cardinal in front of them, because a count of one's own paragraphs drifts the first time one is added: a `command` row has no measured selection, so g8's negation "Not one test the row above selects links `webcam-handler-v4l2`" goes unread; `zero` is out of the lexicon because a row that really selects none goes red at the check above; the lexicon of number *words* stops at `ninety-nine`; and the rate exemption is `per` and `apiece` and deliberately not `each`, which is also the distributive adverb, so exempting it would swallow `the three tests each drive a real refusal` — a true count of the row — in silence. The refusal names the remedy and a second passing arm drives it — say which population the number counts, so that it stops qualifying the bare noun. **Delta (2026-08-21, the review of that repair):** the word reader kept a comma at both *ends* of every word, to protect a thousands separator that only ever stands in the middle of one — so `tests,` was not the noun `tests`, and finding #21's own sentence escaped the ban the moment a clause ran on after it: `the row above selects nine tests, one per hole` passed a seeded table whose row holds one test, with the summary reading `checked 0 count(s)`. The same keeping hid a refusal in the other direction, since a comma'd spelling of g4's signal-parity sentence, `one test, per signal`, reaches the rate exemption with the comma still stuck to `per`, and a repair that read the noun through punctuation and the rate through none would have refused it as a count of one against a row that selects two. A comma is punctuation like any other now and comes off both ends with the full stop; a separator inside a word is untouched, and the arm that holds that is red on the phrase `1,381 tests` as written, so a reader that went back to deleting every comma would name the wrong sentence rather than pass. The hole was load-bearing on the table as it stood: run over it, the two readers disagree by exactly one row, and the row is g8's, which reads "four tests, counted in the prose because a lone `test()` clause carries no alternation for `counted-selections.sh` to split into branches" — a criterion written in this same batch **to be held by this rule**, and not being held by it (this note) | the gate-close review's finding #21: a `what` column said "The row above selects nine tests" of a selection the tool lists ten names for, false on the one commit that ever wrote it, while this predicate measured the true figure and printed it three lines from the sentence it falsified and compared it to nothing but zero. The phrase was deleted (note **N318**) and the class was left with no reader at all. The rule would have gone red on the tree it was written against: `f9abe48`'s g6 chroma row says "each passed **1381 of 1381 tests**", which this reader takes as a claim of 1381 beside a selection `cargo nextest list` answers with 5. It has never fired on the shipped table, because the reword of that cell to `workspace tests` — the remedy the refusal names — was landed by the row audit in the same batch, as this criterion's paired repair, so the tree this predicate has run over never carried the claim; which is why the counterfactual was driven rather than argued, that commit's own cell seeded into a table this predicate reads going red naming `1381 tests` |
...
- **`mutation-scope-is-decided.sh` reads a marker's path and never its reason**, by that
  file's own design ("the reason cell is for the reader"), so nothing goes red when an
  exclusion's stated argument stops being true of the file it excludes. The `facade.rs` entry
  was wrong that way for two batches — it named two mechanisms that reach neither of the two
  exports no CLI verb calls — and what found it was a person running the one command the entry
  should have named (note **N327**). The remedy in place is the register's own law, that an
  exclusion is a decision with a date, plus the measurement written beside it.
