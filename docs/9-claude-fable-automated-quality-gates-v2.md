...
| **Signal parity tests** | P4e-ii | one test per signal (SIGTERM, SIGINT), real delivery, drain asserted with open subscription + mid-flight sweep. **P4e split in two** (note N58): this row is the shutdown half's, and its two ingredients are what makes the split sequential — P4e-i's `crates/daemon/tests/subscriptions.rs` already holds a sweep inside a device write with a subscription open across it, and already ends by asserting `stopped()` resolves | the graceful path production triggers bypassing drain/release |
