...
- **`systemd-units.sh` is text over files, and does not claim systemd would accept the
  units.** It re-derives every value it checks from the crate that owns it, which is the
  transcription defect it exists for; `systemd-analyze verify` is the other check and is
  deliberately not run, because it needs a systemd on the host, it resolves `ExecStart=`
  against a binary that is not installed in a checkout, and its verdict about a missing
  executable is not a verdict about this project. The runtime half — a real `wchd` under a
  real service manager — is `socket-activation.sh`, and between them the units are checked
  for what they *say* and the daemon for what it *does* with what they say.
